...
def _rwr_trace_to_dgl_graph(
    g, seed, trace, positional_embedding_size, entire_graph=False,generate=False
):
    ...
    if generate:
        ...
        if entire_graph:
            ...
            subg.ndata["seed"][0] = 1
    return subg
def _create_dgl_graph(edge_index):
    graph = dgl.DGLGraph()
    ...
    return graph
def _add_feature(seed,graph , positional_embedding_size,entire_graph=False):
    subg = _add_undirected_graph_positional_embedding(graph, positional_embedding_size)
    subg.ndata['_ID'] = torch.tensor([i for i in range(graph.number_of_nodes())])
    subg.ndata["seed"] = torch.zeros(subg.number_of_nodes(), dtype=torch.long)
    subg.ndata["seed"][0] = 1
    subg.edata['_ID'] = torch.tensor([i for i in range(graph.number_of_edges())])
    return subg

def ego_subg(g,seed,positional_embedding_size,ego_hops=2, entire_graph=False) :

    edge_index_in = torch.stack(g.edges())
    ...
    # loader = dgl.dataloading.
    # loader = torch_geometric.data.NeighborSampler(edge_index_in,sizes=[1,2],node_idx =torch.tensor(seed,dtype=int),batch_size=1,num_workers=0)
    # for _,_,adjs in enumerate(loader):
    #     ego_edge_index = adjs[0]
    loader = dgl.contrib.sampling.NeighborSampler(g,1,15,2)
    # torch_geometric.data.NeighborSampler(edge_index_in,sizes =[6,6,6])
    # new_nodes, new_edge_index, new_index, _ = torch_geometric.utils.k_hop_subgraph(int(seed),num_hops=ego_hops,edge_index=edge_index_in,relabel_nodes=True)
    # remain_idxs = random.choice()
    # subgraph =_create_dgl_graph(new_edge_index)
    return ego_edge_index
